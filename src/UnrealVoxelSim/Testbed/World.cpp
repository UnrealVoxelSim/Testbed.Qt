...
{
    const auto result = std::ranges::find(Configurations, id, [](const WorldConfiguration &configuration) {
        return configuration.Descriptor.Id;
    });
...
class World::Impl final
{
  public:
        ...
        : Descriptor(configuration.Descriptor), Profiling(profiling)
    {
        ...
        Movement = std::make_unique<Movement::Voxel::Controller>(*Solids, MovementProfiles);
        NavigationEnvironment =
            std::make_unique<Voxel::Solid::Navigation::Environment>(*Field, *Solids);
        SolidCommands = std::make_unique<Voxel::Solid::Commands::Queue>(*Solids);
        Planner = std::make_unique<Navigation::Voxel::Planner>(
            *NavigationEnvironment, MovementProfiles, Profiling, FineExpansionsPerTick,
            Navigation::Voxel::Planner::DefaultMaximumExpansionsPerRequest, ComponentExpansionsPerTick,
        ...
        Following = std::make_unique<Navigation::Following::Controller>(
            *Planner, *Movement, *Movement, MovementProfiles);
        Pawns = std::make_unique<PawnController>(
            *Entities, *Movement, *Following, *Following, Profiling,
            PawnController::Configuration{MovementProfiles[0].Id, configuration.Bounds, configuration.SpawnRegion,
                                          configuration.SpawnZ, 1, configuration.SpawnSpacing,
    ...
    std::unique_ptr<Ecs::EnTT::Registry> Entities;
    std::unique_ptr<Movement::Voxel::Controller> Movement;
    ...
    std::unique_ptr<Navigation::Following::Controller> Following;
...
std::optional<Movement::Api::State> World::ReadPawn(const Ecs::Api::EntityId entity) const noexcept
{
    const auto state = m_Impl->Movement->Read(entity);
    return state ? std::optional{*state} : std::nullopt;
}

std::optional<Navigation::Api::ExecutionStatus> World::ReadNavigation(const Ecs::Api::EntityId entity) const noexcept
{
    return m_Impl->Following->ReadExecution(entity);
}
...
{
    if (!region.IsValid() || region.IsEmpty() || !material.IsValid()) return false;
    std::vector<Voxel::Solid::Api::Cell> cells(*region.CellCount());
    if (!m_Impl->Solids->ReadRegion(region, cells)) return false;
    ...
    for (auto z = region.Min.Z; z < region.Max.Z; ++z)
        for (auto y = region.Min.Y; y < region.Max.Y; ++y)
            for (auto x = region.Min.X; x < region.Max.X; ++x)
                if (cells[index++].IsEmpty()) placements.push_back({{x, y, z}, material});
...
{
    if (!region.IsValid() || region.IsEmpty()) return false;
    std::vector<Voxel::Solid::Api::Cell> cells(*region.CellCount());
    if (!m_Impl->Solids->ReadRegion(region, cells)) return false;
    ...
    for (auto z = region.Min.Z; z < region.Max.Z; ++z)
        for (auto y = region.Min.Y; y < region.Max.Y; ++y)
            for (auto x = region.Min.X; x < region.Max.X; ++x)
                if (!cells[index++].IsEmpty()) positions.push_back({x, y, z});
...
bool World::Navigate(const Ecs::Api::EntityId pawn, const Voxel::Api::Position destination)
{
    ...
    if (std::ranges::find(pawns, pawn) == pawns.end()) return false;
    constexpr auto one = Movement::Api::Scalar::OneRaw;
    constexpr auto half = one / 2;
    const Movement::Api::Position goal{
        Movement::Api::Scalar::FromRaw(static_cast<std::int64_t>(destination.X) * one + half),
        Movement::Api::Scalar::FromRaw(static_cast<std::int64_t>(destination.Y) * one + half),
        Movement::Api::Scalar::FromRaw(static_cast<std::int64_t>(destination.Z) * one)};
    const std::array commands{Navigation::Api::Command{Navigation::Api::Start{
        {m_Impl->Simulation->CurrentTick(), ManualNavigationSource, ++m_Impl->NavigationSequence}, pawn,
        Navigation::Api::ExecutionId{++m_Impl->NavigationExecution},
        {goal, Movement::Api::Scalar::FromRaw(one / 4)}}}};
    const auto submitted = m_Impl->Following->Submit(commands).has_value();
    if (submitted) m_Impl->Pawns->TrackExternalNavigation(pawn);
    return submitted;
}
...
RuntimeStats World::Stats() const noexcept
{
    ...
    {
        const auto execution = m_Impl->Following->ReadExecution(pawn);
        if (!execution) continue;
        switch (execution->State)
        {
        case Navigation::Api::ExecutionState::Planning: ++result.NavigationCounts[0]; break;
        case Navigation::Api::ExecutionState::Following: ++result.NavigationCounts[1]; break;
        case Navigation::Api::ExecutionState::Replanning: ++result.NavigationCounts[2]; break;
        case Navigation::Api::ExecutionState::Arrived: ++result.NavigationCounts[3]; break;
        case Navigation::Api::ExecutionState::Unreachable: ++result.NavigationCounts[4]; break;
        case Navigation::Api::ExecutionState::Cancelled: ++result.NavigationCounts[5]; break;
        }
    }
    return result;
}

std::span<const WorldDescriptor> WorldCatalog::Worlds() noexcept
{
...
std::unique_ptr<World> WorldCatalog::Create(const std::string_view id, Profiling::Api::IRecorder &profiling)
{
    ...
    if (configuration == nullptr) throw std::invalid_argument{"Unknown testbed world: " + std::string{id}};
