...
class PawnController final : public Simulation::Api::IDecisionUpdater
{
  public:
    ...
    PawnController(Ecs::EnTT::Registry &entities, Movement::Api::ICommands &movement,
  ...
  private:
    ...
    [[nodiscard]] std::int32_t RandomCoordinate(State &state, std::int32_t minimum,
                                                std::int32_t maximum) const noexcept;
    [[nodiscard]] Movement::Api::Position Spawn(std::size_t index) const noexcept;
    [[nodiscard]] Movement::Api::Position Destination(State &state) const noexcept;
    void CreatePawn(std::size_t index);
    void RemovePawn();

    Ecs::EnTT::Registry &m_EntitiesRegistry;
    Movement::Api::ICommands &m_Movement;
    Navigation::Api::ICommandSink &m_NavigationCommands;
    const Navigation::Api::IExecutionReader &m_NavigationExecutions;
